...
async function initDb() {
  ...
  // orders minimal expected columns (لو الجدول موجود قديم مش هيكسره)
  await pool.query(`
...
// ===== LIST UI (Inline Buttons) =====
function ordersListInlineKeyboard(rows) {
  // كل زر = اسم الزبون + التاريخ
...
bot.hears("📋 الأوردرات المسجلة", async (ctx) => {
  // اعرض آخر 20 أوردر (بغض النظر عن الحالة)
...
bot.action(/^order:(\d+)$/, async (ctx) => {
  ...
  if (res.rows.length === 0) {
    return ctx.answerCbQuery("الأوردر مش موجود");
  }
  ...
  // نعرض التفاصيل كرسالة جديدة ونقفل الـ callback
...
bot.on("text", async (ctx) => {
  ...
  // لو المستخدم ضغط زر من المنيو أثناء إدخال الأوردر -> نلغي الجلسة
  if (isMenuText(msg)) {
  ...
  if (s.step === "deposit") {
    ...
    try {
      ...
      sessions.delete(ctx.from.id);
      return ctx.reply("⚠️ حصلت مشكلة في حفظ الأوردر. راجع قاعدة البيانات/الأعمدة.", MENU);
    }
...
// ===== BOOT (تشغيل فورًا + DB في الخلفية) =====
(async () => {
  try {
